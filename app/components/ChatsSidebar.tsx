...
export default function SideChatBar({
  ...
  return (
    <aside>
      <div
        className={`${SideBar ? "w-2/3 md:w-70" : "w-0"} ${Theme ? "bg-[#181818]" : "bg-white"
          } 
      } transition-all duration-300 overflow-hidden flex flex-col border-r ${Theme ? "border-gray-800" : "border-gray-200"
          }`}
      >
          ...
          <div className={`p-3 border-t border-gray-800 flex flex-col `}>
            ...
            >
              <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-sm font-medium">
                {UserData.image ? <img src={UserData.image} alt="user" /> : UserData.name?.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 text-left ">
                ...
                >
                  {UserData.isPremium ? <p>Premium Plan</p> : <p>Free Plan</p>} <p>Credits: {credits.toString()}</p>
                </div>
              </div>
              {isFooterOpen ? (
